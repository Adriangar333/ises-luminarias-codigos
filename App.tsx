...
const App: React.FC = () => {
  ...
  useEffect(() => {
    try {
      const storedImages = localStorage.getItem('processedImages');
  ...
  const processImages = useCallback(async () => {
    ...
    const processingPromises = batchToProcess.map(async (image) => {
      setImages(current => current.map(i => i.id === image.id ? { ...i, status: 'processing' } : i));
      
      try {
        const code = await extractCodeFromImage(image.dataUrl, image.mimeType, effectiveApiKey);
        processedCount++;
        setProcessingMessage(`Procesando ${processedCount} de ${totalInBatch}...`);
        return { id: image.id, status: 'success' as ProcessingStatus, extractedCode: code || 'No encontrado' };
      } catch (error) {
        console.error(`Error processing ${image.fileName}:`, error);
        processedCount++;
        setProcessingMessage(`Error en imagen ${processedCount} de ${totalInBatch}.`);
        const errorMessage = error instanceof Error ? error.message : 'Error desconocido';
        return { id: image.id, status: 'error' as ProcessingStatus, extractedCode: errorMessage };
      }
    });
  ...
  const handleDownloadAll = async () => {
    ...
    for (const image of imagesToDownload) {
        ...
        folder.file(`${finalName}.${extension}`, blob);
    }
